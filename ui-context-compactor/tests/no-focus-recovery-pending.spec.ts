...
import { createUserMessage, LlmAdapter, ReasoningEffortId, type GenerateOptions, type LlmResolvedModelInfo, type StreamChunk, type UserMessage } from '@deepseek-ai/dsh-llm'
...
class Adapter extends LlmAdapter {
  ...
  override resolveModel(provider: string, model: string): Promise<LlmResolvedModelInfo> { return Promise.resolve({ provider, id: model, name: model, context: { contextWindow: 8192 }, reasoning: { efforts: [{ id: ReasoningEffortId('off'), name: 'Off' }] } }) }
...
class Adapter extends LlmAdapter { calls=0; async resolveModel(provider,model){return {provider,id:model,name:model,context:{contextWindow:8192},reasoning:{efforts:[{id:'off',name:'Off'}]}}}; async *stream(options){if(options.messages.some(message=>message.source.kind==='plugin'&&message.source.plugin==='ui-context-compactor:focus-canary-schema')){this.calls++;yield* chunks(this.calls===1?'{"kind":"focus","subject":"child A","relation":"new"}':'{"kind":"close","relation":"current"}');return}yield* chunks('child root receipt')} }
