...
cp "$repository_root/scripts/dsh-web-notify-start-url.mjs" "$source_root/scripts/dsh-web-notify-start-url.mjs"
...
cmp "$source_root/scripts/dsh-web-notify-start-url.mjs" "$remote_root/dsh-web-notify-start-url.mjs"
...
grep -Fq 'fixture.invalid:/home/herman/.local/share/dsh-web-package/dsh-web-notify-start-url.mjs' "$DSH_DEPLOY_COMMAND_LOG"
if grep -Eq 'ssh .*dsh-web-(start|lan-proxy|notify)' "$DSH_DEPLOY_COMMAND_LOG"; then
...
printf '\n' >>"$DSH_START_LOG"
printf 'dsh web: http://127.0.0.1:3080/?token=fixture-launch-token\n'
sleep 0.2
EOF
...
setInterval(() => {}, 1000)
EOF
cat >"$start_root/dsh-web-notify-start-url.mjs" <<'EOF'
import fs from 'node:fs'
const input = fs.readFileSync(0, 'utf8').trim()
fs.appendFileSync(process.env.DSH_START_LOG, `notify-input=${input}\n`)
EOF
...
grep -Fq 'web --host 127.0.0.1 --port 3080 --no-open --trusted-host 192.168.6.240 dsh.man-her.icu' "$DSH_START_LOG"
grep -Fxq 'notify-input=http://127.0.0.1:3080/?token=fixture-launch-token' "$DSH_START_LOG"
...
cp "$repository_root/scripts/dsh-web-start" "$repository_root/scripts/dsh-web-lan-proxy.mjs" "$repository_root/scripts/dsh-web-notify-start-url.mjs" "$missing_node_root/"
...
cp "$repository_root/scripts/dsh-web-start" "$repository_root/scripts/dsh-web-lan-proxy.mjs" "$repository_root/scripts/dsh-web-notify-start-url.mjs" "$missing_root/"
