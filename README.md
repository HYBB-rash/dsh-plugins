...
因此没有可靠的“一行安装命令”。插件请先在隔离环境中作为 DSH/Cordis source plugin 接入，再按其导出的 `name`、`inject`、`Config` 和 `apply()` 连接到宿主 composition；Skill 则放入宿主可发现的 Skill 目录。这里没有声称 `dsh plugin add`、npm 安装或任意 DSH 版本可以直接工作。
