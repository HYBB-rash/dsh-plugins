...
describe('dsh-x-feed package contract', () => {
  it('ships runtime assets without private state or test material', () => {
    ...
    expect(packageJson.name).toBe('@deepseek-ai/dsh-x-feed')
    ...
    const applyStart = rootSource.indexOf('export async function apply')
    expect(applyStart).toBeGreaterThanOrEqual(0)
    const applySource = applyStart < 0 ? '' : rootSource.slice(applyStart)
    expect(applySource).not.toContain('createFactProjectionPreflight')
    expect(applySource).not.toContain('preflightFactProjection')
