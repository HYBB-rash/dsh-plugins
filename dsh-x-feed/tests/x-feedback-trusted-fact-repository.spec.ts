...
describe('FileTrustedFactRepository', () => {
  ...
  it('does not create locators for corrupt, unknown, or unadmitted candidate rows', async () => {
    ...
    expect(warnings).toEqual([
      `dsh-x-feed: skipping corrupt trusted-facts.jsonl line: ${invalid}`,
      `dsh-x-feed: skipping invalid trusted-facts.jsonl line: ${unknown.slice(0, 120)}`,
      `dsh-x-feed: skipping invalid trusted-facts.jsonl line: ${candidate.slice(0, 120)}`,
    ])
  })
