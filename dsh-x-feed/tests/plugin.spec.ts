...
describe('feedback context contract (§10.3)', () => {
  it('只处理可定位的 X 反馈：多 URL 歧义只问一句，成功写入后确认且不扩展为其他责任', () => {
    ...
    expect(X_FEED_CONTRACT).toContain('喜欢、不喜欢、收藏或取消收藏时，先定位目标，再调用对应工具（x_feed_record_feedback）')
    expect(X_FEED_CONTRACT).toContain('只有工具返回成功后，才能向用户确认')
    expect(X_FEED_CONTRACT).toContain('只进 X 反馈账本，不进长期 canary memory')
