...
function linkWorkspaceDependencies(nodeModules: string): void {
  const sourceScope = join(workspaceDirectory, 'node_modules', '@deepseek-ai')
  ...
  for (const entry of readdirSync(sourceScope)) {
    if (entry === 'dsh-cron' || entry === 'dsh-x-feed') continue
    symlinkSync(join(sourceScope, entry), join(targetScope, entry), 'dir')
  }
  const standardSchema = join(workspaceDirectory, 'node_modules', '@standard-schema')
...
describe('TODO7 packed NodeNext public-root consumer', () => {
  it('packs both packages and executes a consumer against only their public roots', () => {
    ...
    const xFeedPackage = join(consumerNodeModules, '@deepseek-ai/dsh-x-feed')
    mkdirSync(tarballs)
    ...
    expect(xFeedManifest.name).toBe('@deepseek-ai/dsh-x-feed')
    ...
    assertNoCrossPackageSubpathImports(consumer, '@deepseek-ai/dsh-x-feed')
