...
function pluginCompositionContext(registry: unknown): {
...
describe('dsh-x-feed/v1 cron provider composition boundary', () => {
  ...
  it('registers only when cron is bound and disposes exactly its provider registration', async () => {
    const directory = await temporaryDirectory()
    const registry = createCronAgentEnvironmentRegistry()
    const fixture = pluginCompositionContext(registry)
    await apply(fixture.context as never, {
      cronJobId: 'cron-x',
      dataDir: directory,
      pythonBin: 'python3',
      pipelinePath: '/pkg/python/x_insight_pipeline.py',
    ...
    const emptyRegistry = createCronAgentEnvironmentRegistry()
    const emptyFixture = pluginCompositionContext(emptyRegistry)
    await apply(emptyFixture.context as never, { cronJobId: '' } as never)
    expect(emptyRegistry.resolve(X_CRON_AGENT_ENVIRONMENT_MARKER)).toMatchObject({
      ok: false,
      error: { code: 'missing_provider' },
    })
  })
