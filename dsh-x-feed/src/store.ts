/**
 * dsh-x-feed feedback ledger: append-only `feedback.jsonl` under the Harness
  ...
  readAll(warn?: (message: string) => void): XFeedbackEvent[] {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        warn?.(`dsh-x-feed: skipping corrupt feedback.jsonl line: ${trimmed.slice(0, 120)}`)
      }
    }
    return events
  }
  ...
  private appendLine(event: XFeedbackEvent): void {
    ...
    try {
      ...
      if (written !== Buffer.byteLength(line)) {
        throw new Error(`dsh-x-feed: incomplete feedback append (${written}/${Buffer.byteLength(line)} bytes)`)
      }
