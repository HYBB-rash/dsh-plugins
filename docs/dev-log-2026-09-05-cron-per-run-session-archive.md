...
- 本轮没有合并、推送当前修复分支、发布、重启或写入远端 112 个历史归档；这些仍需用户后续分别授权。
- 上线前必须重新只读生成本机和 `herman.hermes` 完整候选清单；若数量、终态分布或哈希身份与已确认基线不同，停止生产写入并重新核对。
- 真实 Telegram、两个 active `per_run` job 和远端历史收敛只能在获准发布与重启后验收；本地配置合成不等同于生产业务健康。
