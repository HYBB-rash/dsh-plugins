...
- 日期：2026-09-05
- 范围：本地 Web 运行入口、入口合同测试；不修改 upstream，不改动现有会话数据
...
已有旧会话不能靠普通 Workspace 绑定迁移。若需要保留历史并改变其 Workspace，必须在服务停止时迁移会话的持久化身份与存储位置，并另做快照和完整性验证；这不属于本次无数据改写的运行入口修复。
...
本地 `.dsh-web` 和正式 `~/.dsh` 中已经存在、header 仍记录旧目录的 `session-telegram` 不会由本次代码自动改写。历史迁移必须作为单独的数据操作，在精确列出目标、完成快照并确认停服后执行。
