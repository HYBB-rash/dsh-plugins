...
describe('gateway lifecycle', () => {
  ...
  it('sends a handled-awaiting-delivery final first, then settles exactly once with its real receipt, without entering root', async () => {
    scratch = mkdtempSync(join(tmpdir(), 'tg-gateway-'))
    let settledReceipt: unknown
    const order: string[] = []
    const harness = gatewayContext({
      credentialValues: ['42'],
      inbound: {
    ...
    const sendMessage = vi.fn(async (_chatId: number, text: string, _options?: SendMessageOptions) => {
      if (text === 'Feed结果') order.push('send')
      return { messageId: text === 'Feed结果' ? 99 : 1 }
    })
    let polls = 0
    const http: TelegramHttp = {
      getMe: async () => ({ id: 1 }),
    ...
    expect(harness.agent.followup).not.toHaveBeenCalled()
    expect(order).toEqual(['send', 'settle'])
    expect(settledReceipt).toEqual({
      chatId: 42,
      triggerMessageId: 5,
      visibleText: 'Feed结果',
      messageIds: [99],
  ...
  it('does not settle or send a generic second terminal when the handled final send fails', async () => {
    scratch = mkdtempSync(join(tmpdir(), 'tg-gateway-'))
    let settleCalls = 0
    const harness = gatewayContext({
      credentialValues: ['42'],
    ...
    const lifetime = new AbortController()
    const sendMessage = vi.fn(async (_chatId: number, text: string, _options?: SendMessageOptions) => {
      if (text === 'Feed结果') throw new Error('final send failed')
      return { messageId: 1 }
    })
    let polls = 0
    const http: TelegramHttp = {
  ...
  it('does not send a second terminal when settlement throws after a successful handled final', async () => {
    scratch = mkdtempSync(join(tmpdir(), 'tg-gateway-'))
    let settleCalls = 0
    const harness = gatewayContext({
      credentialValues: ['42'],
    ...
    const lifetime = new AbortController()
    const sendMessage = vi.fn(async (_chatId: number, _text: string, _options?: SendMessageOptions) => ({ messageId: 99 }))
    let polls = 0
    const http: TelegramHttp = {
      getMe: async () => ({ id: 1 }),
      getUpdates: async () => {
