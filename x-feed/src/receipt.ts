/**
 * X feed delivery receipt (§11): maps dsh-cron's final per-run receipt
 ...
 * backoff, at most 3 attempts; exhaustion throws so dsh-cron records a bounded
 * settlement error — Telegram is never re-delivered and shown is never touched.
...
export class DeliveryReceipt {
  ...
  async handle(event: CronRunFinishedEvent): Promise<ReceiptResult> {
    if (this.deps.cronJobId === '') {
      this.deps.logger.warn('x-feed: receipt 未绑定 cronJobId，忽略终态事件')
  ...
  async confirm(status: 'delivered' | 'not-delivered'): Promise<ReceiptResult> {
    ...
    for (let attempt = 1; attempt <= DeliveryReceipt.MAX_ATTEMPTS; attempt++) {
      try {
        ...
        this.deps.logger.warn(`x-feed: confirm-prepared ${status} attempt ${attempt} failed: ${lastError}`)
    ...
    throw new Error(
      `x-feed: confirm-prepared ${status} failed after ${DeliveryReceipt.MAX_ATTEMPTS} attempts: ${lastError}`,
    )
  }
