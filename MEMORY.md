...
- 2026-08-31：`dsh harness notion-automation --approved` 的 CLI 通道曾稳定复现"ssh 退出码 4"（远程 loader reject），而同 payload 经 scp+ssh 手动执行同一 loader 成功完成安装——该 CLI 通道异常未定位（目标 create-only 安装后无法再复测）；任何复测需在目标删除后重新授权。安装回执与文件身份（entrypointSha256/12 测试全真/probeSha256=仓库值）独立复核无误。
