...
- 2026-09-05：cron_list 的 execute 若返回可选 cwd，output.schema 必须同步声明；additionalProperties:false 会把整个列表报成 oneOf matched 0。应测试真实 SDK 对执行结果的校验，不能只测 execute 返回值。cron 的 success/delivered 也不代表业务执行：缺少 standard 预设时，会话可能只有三个 cron 管理工具并不断重新排期；要核对 request/header.tools 与真实报告正文。详见 docs/dev-log-2026-09-05-cron执行工具与列表输出.md。
- 2026-09-05：迁移官方 npm 后，同一个会话目录可能同时保留旧 session.v2.jsonl.zstd 与当前 session.jsonl.zstd；应根据最新事件时间和正在使用的格式核对实际请求工具。当前修复线上已通过一次现有 cron_run_now：bash 执行、原文摘录、最终报告和 Telegram delivered 串联验证，重跑前后 jobs.jsonl 哈希不变。详见 docs/dev-log-2026-09-05-cron线上修复验收.md。
- 2026-09-05：官方 npm 迁移后的本机部署以当前脚本和 unit 为准：dsh-web-local-deploy 只在 incoming/batch.* 准备普通归档，不再停服、快进 checkout 或启动。现役 dsh-web-local.service 指向 dsh-web-package-local/current/bin/web，凭据必须取本机副本；此前本文件关于旧自动快进部署脚本的经验已失效。详见 docs/dev-log-2026-09-05-cron本机主线上线.md。
- 2026-09-05：本机批次目录与服务包根目录必须分开：新增 `dsh-web-local-deploy install <batch>` 在已停服检查后，使用批次安装器的 `--archive-dir` 读取归档，但更新包根目录 `current`，不再把指针留在 incoming 内。prepare 仍只准备且必须显式指定本机凭据；install 不停止、不启动、不重新选版，失败保留旧指针但不保证 Profile 回滚。此条补充上一条“只准备”的范围，详见 docs/dev-log-2026-09-05-local-deploy-handoff.md。
