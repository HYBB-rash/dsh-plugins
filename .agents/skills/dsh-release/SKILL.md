...
1. Run `./release/dsh release --candidate <candidate.json>` without `--approved-stop`.
2. Report the exact candidate and image identity, target, writers to stop, snapshot destination, rollback boundary, and expected outage work.
3. Stop at the authorization gate. An initial request to “release” or “上线” expresses intent but is not the post-plan authorization to stop production.
