...
#!/usr/bin/env bash
set -Eeuo pipefail
printf 'build tool=%s setpriv_marker=%s\n' "$(basename "$0")" "${DSH_VERIFY_SETUID:-}" >>"$MOCK_VERIFY_LOG"
EOF
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf 'python setpriv_marker=%s home=%s data=%s node_path=%s\n' \
  "${DSH_VERIFY_SETUID:-}" "$HOME" "$DSH_X_FEED_DATA_DIR" "${NODE_PATH:-unset}" >>"$MOCK_VERIFY_LOG"
EOF
