...
packages=(telegram-gateway dsh-cron dsh-assistant personal-feed-selector personal-feed x-feed ui-context-compactor)
requested_package="${1:-all}"

case "$requested_package" in
  all) selected_packages=("${packages[@]}") ;;
  telegram-gateway|dsh-cron|dsh-assistant|personal-feed-selector|personal-feed|x-feed|ui-context-compactor)
